...
const getAllData = async (req, res) => {
  try {
    const crud = await adminMessageModel.find({})
...
const createData = async (req, res) => {
  try {
    const crud = await adminMessageModel.create(req.body)
    res.status(200).json({ crud })
  } catch (error) {
    res.status(500).json({ message: error })
  }
...
const getOneData = async (req, res) => {
  try {
    const { messageId: crudId } = req.params
    console.log(req.params)
    // console.log(foodId)
    // console.log(crudId)
    const crud = await adminMessageModel.find({ trainer_id: crudId })
...
const deleteData = async (req, res) => {
  try {
    const { messageId: crudId } = req.params
    const crud = await adminMessageModel.findByIdAndDelete({ _id: crudId })
