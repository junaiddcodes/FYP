...
const getAllData = async (req, res) => {
  try {
    const crud = await gymDetails.find({ listed: "listed" }).sort({numReview: -1}).limit(9);
  ...
  try {
    ...
    if (rating > 5 || rating < 1) {
      return res.status(404).json({
        message: "Rating should not be greater than 5 or less than 1",
      });
    }

    const user = await customerDetails.findById(user_id);
    if (!user) {
